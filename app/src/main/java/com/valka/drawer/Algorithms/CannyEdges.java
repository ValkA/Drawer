package com.valka.drawer.Algorithms;
...
public class CannyEdges {
    ...
    private void readLuminance() {
        ...
        if (type == Bitmap.Config.ARGB_8888) {
            for(int y=0; y<height; ++y) {
                for (int x = 0; x < width; x++) {
                    int p = sourceImage.getPixel(x,y);
                    data[y*width+x] = luminance(Color.red(p), Color.green(p), Color.blue(p));
                }
            }
